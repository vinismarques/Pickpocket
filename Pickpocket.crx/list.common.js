var itemList = {
  ...
  select: function (li, index, noSave) {
    ...
    if (this.selectedLi) {
      ...
      this.selectedLi.classList.remove('showurl');
    }
    ...
    if (this.selectedLi) {
      ...
      this.urlRevealTimer = setTimeout(function () {
        itemList.selectedLi.classList.add('showurl');
  ...
  toggleArchivedUI: function (li) {
    if (this.selectedLi.classList.contains('archived')) {
      li.classList.replace('archived', 'queued');
    } else {
      li.classList.replace('queued', 'archived');
    }
  },
...
function initialize() {
  ...
  processing      = false;
  searchWaiter    = null;
  maxWinHeight    = chrome ? window.innerHeight : 600;
  ...
  if (lin) document.body.classList.add('lin');
  if (mac) document.body.classList.add('mac');
  if (win) document.body.classList.add('win');
  if (xp)  document.body.classList.add('xp');

  if (chrome) {
    document.body.classList.remove('initial');
    document.body.style.height = maxWinHeight - 16 + 'px';
  }
